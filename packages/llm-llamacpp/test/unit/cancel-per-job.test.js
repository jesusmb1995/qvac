...
test('cancel() on one concurrent response cancels only that job, not all', async (t) => {
  ...
  t.is(model.addon.cancel.called, false,
    'whole-model cancel() must NOT be used to cancel a single job')
  t.is(model.addon.cancelJob.callCount, 1,
    'per-job cancelJob() must be invoked exactly once')
  t.alike(model.addon.cancelJob.lastArgs, [1],
