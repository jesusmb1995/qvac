...
test('continuous batching: independent concurrent run() calls stay isolated and decode in parallel', { timeout: 900_000, skip: skipHeavyPlatform }, async t => {
  const model = await setupModel(t, { parallel: '4' })

  const cases = ['capital-france', 'sky-color', 'bee-product', 'frozen-water']
  ...
  const maxConcurrentSeq = Math.max(0, ...responses.map(r => toNumber(r?.stats?.avgConcurrentSeq)))
  t.comment(`max avgConcurrentSeq across responses: ${maxConcurrentSeq}`)
  t.ok(maxConcurrentSeq > 1, `avgConcurrentSeq (${maxConcurrentSeq}) > 1 confirms multi-job decode concurrency`)
})
