...
inline js_value_t* runJob(js_env_t* env, js_callback_info_t* info) try {
  ...
  // The job id is minted here, native-side, so the JS caller never supplies or
  // tracks one. Streamed output is tagged with it and the id is handed back in
  // the result, mirroring how batch ids are assigned above. Starts at 1 to stay
  // clear of kNoJobId (0). Monotonic across instances, which is harmless: the JS
  // side maps each returned id within its own instance.
  static std::atomic<JobId> nextJobId{1};
  const JobId jobId = nextJobId.fetch_add(1, std::memory_order_relaxed);
