...
const { loadSupertonicTTS, runSupertonicTTS, runSupertonicSentenceStream } = require('../utils/runSupertonicTTS')
...
test('Supertonic TTS: Sentence stream (runSentenceStream + onUpdate)', { timeout: 1800000 }, async (t) => {
  const baseDir = getBaseDir()
  const modelDir = path.join(baseDir, 'models', 'supertonic')
  ...
  const result = await runSupertonicSentenceStream(
    model,
    {
      text,
      saveWav,
